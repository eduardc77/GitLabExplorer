...
extension GitLabAPI {
  class GetUserContributedProjectsQuery: GraphQLQuery {
    static let operationName: String = "GetUserContributedProjects"
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"query GetUserContributedProjects($first: Int = 20, $after: String) { projects(first: $first, after: $after, membership: true) { __typename pageInfo { __typename hasNextPage hasPreviousPage startCursor endCursor } edges { __typename cursor node { __typename id name path fullPath description visibility avatarUrl webUrl starCount forksCount lastActivityAt createdAt updatedAt repository { __typename rootRef empty } namespace { __typename id name path fullPath avatarUrl webUrl } } } } }"#
      ))
